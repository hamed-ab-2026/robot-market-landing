...
export default function AuthModal() {
    ...
    const submit = async (values: {phone?: string; code?: string; password?: string}) => {
        ...
        try {
            ...
            setError(caught instanceof Error && caught.message === 'INVALID_PHONE' ? copy.invalidPhone : copy.invalidCredentials);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <Modal open={auth.loginOpen} onCancel={auth.closeLogin} footer={null} centered title={copy.login} destroyOnClose>
            <Segmented<LoginMode> block value={mode} onChange={value => {setMode(value); setStep('phone'); setError('');}}
                options={[{label: copy.otpLogin, value: 'otp'}, {label: copy.passwordLogin, value: 'password'}]}/>
            <Alert className="mt-5" type="info" showIcon message={copy.demoTitle}
                description={<div><div>{copy.demoOtp}</div><div>{copy.demoPassword}</div></div>}/>
            {error && <Alert className="mt-4" type="error" showIcon message={error}/>} 
            <Form layout="vertical" className="mt-5" onFinish={submit} initialValues={{phone: '09123456789', code: '12345', password: '123456'}}>
                {(mode === 'password' || step === 'phone') && (
                    <Form.Item name="phone" label={copy.phone} rules={[{required: true, pattern: /^09\d{9}$/, message: copy.invalidPhone}]}>
                ...
                {mode === 'otp' && step === 'code' && (
                    <>
                        <Typography.Paragraph type="secondary">{copy.phone}: <span dir="ltr">{phone}</span></Typography.Paragraph>
