...
export default function ProtectedPage({children}: {children: ReactNode}) {
    ...
    if (!auth.ready) return <main className="max-w-5xl mx-auto min-h-[70vh] px-5 pt-32"><Skeleton active/></main>;
    if (!auth.isAuthenticated) return (
        <main className="max-w-3xl mx-auto min-h-[70vh] px-5 pt-32 pb-20 text-center">
            <div className="rounded-3xl border border-subtle bg-surface p-10">
                ...
                <Button type="primary" size="large" onClick={() => auth.openLogin()}>{copy.openLogin}</Button>
            </div>
        </main>
    );
    return children;
