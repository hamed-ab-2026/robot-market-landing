    ...
    <div className="horizontal-panel flex items-center justify-center px-6 md:px-16" dir={dir}>
      <div className="relative z-10 max-w-6xl w-full grid md:grid-cols-2 gap-10 items-center">
        <div
          ...
          className="spotlight-stage relative h-[420px] md:h-[560px] flex items-center justify-center"
        >
          ...
          <span className="absolute top-3 z-30 text-[11px] px-3 py-1 rounded-full bg-brand/15 border border-subtle text-brand-400 start-3">
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="grid grid-cols-[auto,1fr] gap-4 items-start bg-surface border border-subtle rounded-2xl p-4">
            <img
              ...
              className="w-28 h-20 md:w-32 md:h-24 object-cover rounded-lg border border-subtle"
            />
            <p className="text-sm md:text-base text-secondary leading-7">{machine.description}</p>
          </div>

          <ul className="flex flex-wrap gap-2">
            {machine.highlights.map((h) => (
              <li key={h} className="text-xs md:text-sm text-brand-400 bg-brand/10 border border-subtle rounded-full px-3 py-1">
                {h}
              </li>
            ))}
          </ul>
