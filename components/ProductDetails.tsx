        ...
        <main className="max-w-7xl mx-auto px-5 md:px-8 pt-28 pb-20">
            ...
            <div className="grid md:grid-cols-2 gap-10 lg:gap-16 items-start">
                <section aria-label={copy.gallery} className="rounded-3xl border border-subtle bg-surface p-6">
                    <img
                        src={alternate ? machine.imageAlt : machine.image}
                        alt={`${machine.name} — ${alternate ? copy.alternateImage : copy.baseImage}`}
                    ...
                    <div className="flex justify-center gap-3 mt-6">
                        {[false, true].map(value => (
                            <button
                                key={String(value)}
                                onClick={() => setAlternate(value)}
                                aria-pressed={alternate === value}
                            ...
                            >
                                <img
                                    src={value ? machine.imageAlt : machine.image}
                                    alt=""
                ...
                <section>
                    ...
                    <h1 className="text-3xl lg:text-4xl font-extrabold text-primary mt-3 leading-relaxed">
                        {machine.name}
                    </h1>
                    <p className="text-secondary mt-5 leading-8">{machine.description}</p>
                    <p className="text-sm text-secondary mt-4">
                        {copy.model}: <span dir="ltr">{machine.id.toUpperCase()}</span>
                    </p>
                    <h2 className="font-bold text-primary text-xl mt-8 mb-4">{copy.specifications}</h2>
                    <ul className="space-y-3">
                        {machine.highlights.map(feature => (
                            <li key={feature} className="flex gap-3 text-secondary">
                                <CheckCircleOutlined className="text-brand-400" />
                                {feature}
                            </li>
                        ))}
                    </ul>
                    <div className="border-t border-subtle mt-8 pt-6">
                        ...
                        <div className="mt-5 flex flex-wrap gap-4 items-center">
                            ...
                            <Link href="/cart" className="text-brand-400 font-semibold hover:underline">
                                {copy.cartLink}
                            </Link>
                        </div>
