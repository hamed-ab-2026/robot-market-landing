...
function MapThumb({ coords, mapStyle, alt }) {
  ...
  if (!NESHAN_API_KEY || errored) {
    return (
      <div className="w-full h-full min-h-[280px] flex flex-col items-center justify-center gap-3 bg-surface text-secondary text-sm text-center p-6">
...
export default function MapSection() {
  ...
  const pins = [
    { key: 'factory', label: t.map.factoryLabel, coords: mapLocations.factory, address: t.contactInfo.factoryAddress },
    { key: 'sales', label: t.map.salesLabel, coords: mapLocations.salesOffice, address: t.contactInfo.salesOfficeAddress },
  ];
            ...
            {pins.map((pin) => (
              <div key={pin.key} className="bg-surface border border-subtle rounded-2xl p-5">
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 shrink-0 rounded-lg bg-brand/15 border border-subtle flex items-center justify-center text-brand-400">
                    <EnvironmentOutlined />
                  </div>
                  <div>
