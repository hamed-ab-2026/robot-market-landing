    ...
    const cards = [
        {
            ...
            value: `${completion.toLocaleString(locale === 'fa' ? 'fa-IR' : 'en-US')}٪`,
            icon: <CheckCircleOutlined />,
            extra: <Progress percent={completion} showInfo={false} size="small" />,
        },
        {
            label: copy.activeOrders,
            value: activeOrders.toLocaleString(locale === 'fa' ? 'fa-IR' : 'en-US'),
            icon: <ClockCircleOutlined />,
        },
        {
            label: copy.totalOrders,
            value: orders.length.toLocaleString(locale === 'fa' ? 'fa-IR' : 'en-US'),
    ...
    return (
        <>
            <h1 className="text-3xl font-extrabold text-primary">
                {copy.welcome} {auth.session?.profile.firstName}
            </h1>
